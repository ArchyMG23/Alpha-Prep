  ...
  return (
    <div className="space-y-8 md:space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-4 md:px-0">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Méthodologie de Travail</h1>
          <p className="text-slate-500 mt-2 text-base md:text-lg font-medium">Maîtrisez les stratégies gagnantes pour chaque module.</p>
        </div>
        ...
        {categories.map(cat => {
          ...
          return (
            <div key={cat.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              <div className={`p-5 md:p-6 ${cat.bg} border-b border-slate-100 flex items-center justify-between`}>
                <div className="flex items-center gap-3 md:gap-4">
                  <div className={`p-2.5 md:p-3 bg-white rounded-2xl shadow-sm ${cat.color}`}>
                    <cat.icon size={22} />
                  </div>
                  <h3 className="text-lg md:text-xl font-black text-slate-900 tracking-tight">{cat.label}</h3>
                </div>
                {isLocked && (
                  <span className="px-2.5 py-1 bg-amber-100 text-amber-700 rounded-full text-[9px] md:text-[10px] font-black tracking-widest uppercase flex items-center gap-1">
                    ...
                    <div key={item.id} className="group cursor-pointer">
                      <div className={`p-4 rounded-2xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all flex items-center justify-between ${isLocked ? 'opacity-50 grayscale' : ''}`}>
                        <div className="overflow-hidden">
                          ...
                          <p className="text-[10px] md:text-xs text-slate-500 font-medium line-clamp-1">{item.content}</p>
                        </div>
                        <ChevronRight size={18} className="text-slate-300 group-hover:text-indigo-500 transition-colors shrink-0 ml-2" />
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="py-8 md:py-10 text-center text-slate-400">
                    <p className="text-xs md:text-sm font-bold italic">Bientôt disponible pour {selectedTest}</p>
                  </div>
                )}
              </div>
          ...
          <div className="flex-1">
            ...
            <div className="space-y-5 md:space-y-6">
              <div className="flex gap-4">
                <div className="w-8 h-8 bg-indigo-800 rounded-lg flex items-center justify-center shrink-0 font-black text-indigo-300 text-sm">1</div>
                <p className="font-medium text-indigo-100 text-sm md:text-base">Pratiquez quotidiennement au moins 30 minutes sur chaque module.</p>
              </div>
              <div className="flex gap-4">
                <div className="w-8 h-8 bg-indigo-800 rounded-lg flex items-center justify-center shrink-0 font-black text-indigo-300 text-sm">2</div>
                <p className="font-medium text-indigo-100 text-sm md:text-base">Écoutez des podcasts en français/anglais pour habituer votre oreille aux accents.</p>
              </div>
              <div className="flex gap-4">
                <div className="w-8 h-8 bg-indigo-800 rounded-lg flex items-center justify-center shrink-0 font-black text-indigo-300 text-sm">3</div>
                <p className="font-medium text-indigo-100 text-sm md:text-base">Utilisez notre simulateur IA pour corriger vos fautes récurrentes.</p>
              </div>
            </div>
          </div>
          <div className="w-full lg:w-1/3 bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-white/20">
            <h4 className="font-black mb-4 uppercase tracking-widest text-[10px] text-indigo-200">Ressources Externes</h4>
            <ul className="space-y-4">
              <li className="flex items-center justify-between group cursor-pointer">
                <span className="text-xs md:text-sm font-bold">TV5 Monde TCF</span>
                <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </li>
              <li className="flex items-center justify-between group cursor-pointer">
                <span className="text-xs md:text-sm font-bold">RFI Savoirs</span>
                <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </li>
              <li className="flex items-center justify-between group cursor-pointer">
                <span className="text-xs md:text-sm font-bold">IELTS British Council</span>
                <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </li>
            </ul>
          </div>
