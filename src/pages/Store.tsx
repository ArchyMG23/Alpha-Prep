...
export default function Store() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in relative pb-20">
      <div className="text-center max-w-3xl mx-auto">
        <h1 className="text-4xl font-black text-slate-900 tracking-tight">Boutique Officielle</h1>
        <p className="text-slate-500 mt-4 text-xl font-medium">Achetez vos accès via WhatsApp et recevez votre clé d'activation instantanément.</p>
      </div>

      {/* Access Key Activation */}
      <div className="max-w-2xl mx-auto bg-indigo-900 p-8 rounded-3xl shadow-xl shadow-indigo-200 text-white">
        ...
        <div className="flex gap-4">
          <input 
            ...
            placeholder="ENTREZ VOTRE CLÉ ICI (ex: AB12CD34)"
            className="flex-1 px-6 py-4 bg-indigo-800 border border-indigo-700 rounded-2xl outline-none focus:border-white transition-all font-mono font-bold placeholder:text-indigo-400"
          />
          <button 
            onClick={handleActivateKey}
            className="px-8 py-4 bg-white text-indigo-900 font-black rounded-2xl hover:bg-indigo-50 transition-all shadow-lg"
          >
            Activer
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {filteredPrices.map(plan => (
          <div key={plan.id} className="bg-white p-6 rounded-3xl border-2 border-slate-100 transition-all flex flex-col hover:border-indigo-200 hover:shadow-xl">
            ...
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-4">Accès {plan.accessLevel}</p>
            <div className="text-3xl font-black text-slate-900 mb-6">{plan.priceXAF.toLocaleString()} <span className="text-sm">FCFA</span></div>
            ...
            <button 
              ...
              className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2"
            >
      ...
      <div className="bg-white p-10 rounded-3xl border border-slate-200 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">Corrections chirurgicales valables pour TOUS les tests.</p>
          </div>
          <div className="flex items-center gap-3 text-sm font-black text-indigo-600 bg-indigo-50 px-5 py-2.5 rounded-full border border-indigo-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={pack.amount} className="p-8 rounded-3xl border-2 border-slate-100 hover:border-indigo-200 transition-all flex flex-col items-center text-center">
              <div className="text-xs font-black text-slate-400 mb-4 uppercase tracking-widest">{pack.desc}</div>
              <div className="text-5xl font-black text-slate-900 mb-1 tracking-tighter">{pack.amount}</div>
              <div className="text-slate-500 text-xs font-bold mb-8 uppercase tracking-widest">Crédits</div>
              <div className="text-2xl font-black text-slate-900 mb-8">{pack.price.toLocaleString()} FCFA</div>
