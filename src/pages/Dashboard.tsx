  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Bonjour, {user.name.split(' ')[0]}</h1>
          <p className="text-slate-500 mt-2 text-lg font-medium italic">"Le succès est la somme de petits efforts répétés jour après jour."</p>
        </div>
        <div className="flex flex-wrap gap-3">
          {activeSubs.length > 0 ? (
            activeSubs.map(sub => (
              <span key={sub.testType} className="px-4 py-2 bg-indigo-600 text-white rounded-full text-[10px] font-black tracking-widest uppercase shadow-lg shadow-indigo-200 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col group">
          ...
          <div className="mt-auto">
            <div className="flex items-baseline gap-2">
              <span className="text-5xl font-black text-slate-900 tracking-tighter">{user.estimatedCRS}</span>
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col group">
          ...
          <div className="mt-auto">
            <div className="flex items-baseline gap-2">
              <span className="text-5xl font-black text-slate-900 tracking-tighter">{user.averageCLB.toFixed(1)}</span>
              <span className="text-sm text-slate-400 font-bold uppercase tracking-widest">Moyen</span>
            </div>
            <p className="text-xs text-slate-400 font-bold mt-6 uppercase tracking-widest">Objectif : NCLC 7+</p>
          </div>
        </div>

        <div className="bg-indigo-900 p-8 rounded-3xl shadow-xl shadow-indigo-900/20 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <span className="text-5xl font-black text-white tracking-tighter">{user.correctionCredits}</span>
          </div>
          <button 
            onClick={() => navigateTo('store')}
            className="relative z-10 w-full mt-8 py-3 bg-white hover:bg-indigo-50 text-indigo-900 font-black rounded-2xl transition-all text-sm uppercase tracking-widest"
          >
            Recharger
          </button>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
          ...
          <button onClick={() => navigateTo('exam')} className="text-indigo-600 text-sm font-black uppercase tracking-widest hover:text-indigo-700 flex items-center gap-2 group">
        ...
        {attempts.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {attempts.map(attempt => (
              <div key={attempt.id} className="p-6 px-8 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400">
                    <PenTool size={20} />
                  </div>
                  <div>
                    <p className="font-bold text-slate-900">Simulation #{attempt.questionId.substring(0,4)}</p>
                ...
                <div className="flex items-center gap-4">
                  <span className="px-4 py-2 bg-emerald-50 text-emerald-700 rounded-full text-sm font-black tracking-tighter shadow-sm border border-emerald-100">
                    {attempt.scoreCLB}
                  </span>
                </div>
              </div>
